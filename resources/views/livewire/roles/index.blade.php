    ...
    <section class="content-panel px-7 py-7 sm:px-10 sm:py-8">
        <div class="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
            <div>
                ...
                <h2 class="mt-5 text-3xl font-semibold text-slate-100 md:text-4xl">Roles & Permissions</h2>
    ...
    <section class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <div class="table-card overflow-hidden">
            <div class="border-b border-slate-800 px-8 py-5">
                <h3 class="text-lg font-semibold text-slate-100">Roles</h3>
            </div>
            <div class="divide-y divide-slate-800">
                @foreach ($roles as $role)
                    <div class="flex flex-col gap-4 px-8 py-5 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            <p class="text-lg font-semibold text-slate-100">{{ $role->label }}</p>
                            <p class="text-sm uppercase tracking-[0.24em] text-slate-500">{{ $role->name }}</p>
                            <p class="mt-2 text-sm text-slate-400">{{ $role->users_count }} users • {{ $role->permissions->count() }} permissions</p>
                        </div>
                        <div class="flex gap-2">
                            <button type="button" wire:click="edit({{ $role->id }})" class="btn-secondary px-4 py-2.5">Edit</button>
                            <button type="button" wire:click="delete({{ $role->id }})" class="btn-danger px-4 py-2.5">Delete</button>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="table-card overflow-hidden">
            <div class="border-b border-slate-800 px-8 py-5">
                <h3 class="text-lg font-semibold text-slate-100">Available permissions</h3>
            </div>
            <div class="space-y-6 px-8 py-6">
                @foreach ($permissions as $group => $groupPermissions)
                    <div>
                        <p class="text-sm uppercase tracking-[0.26em] text-slate-500">{{ $group }}</p>
                        ...
                        @foreach ($permissions as $group => $groupPermissions)
                            <div class="rounded-3xl border border-slate-800 p-5">
                                ...
                                <div class="mt-4 grid gap-3 sm:grid-cols-2">
                                    @foreach ($groupPermissions as $permission)
                                        <label class="flex items-center gap-3 rounded-2xl border border-slate-800 px-4 py-3 text-sm text-slate-300">
